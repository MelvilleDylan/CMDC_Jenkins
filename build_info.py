...
def load_job_details(file_path = None):
...
def generate_builds_dataframe(jobs):
    ...
    for build in builds_1d:
        ...
        add_to_data = [data[col].append(build.details[col]) for col in build.details]
...
class Build:
    def __init__(self, job, number):
        ...
        self.full_log = self.get_log(self.log_url)
    ...
    def get_log(self,log_url):
        web_request = requests.get(log_url)
    ...
    def scrape_log(self):
        #Now we pull all the test summary results table info
    ...
    def scrape_details(self):
        ...
        #   connect_ping
        site_code = re.search('Site Code:', self.full_log)
        return site_code
...
class Job:
    ...
    def output_to_csv(self,file_location):
